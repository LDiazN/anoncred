...
from anoncred.protocol import Issuance, PRF, get_nym_id, compute_cred_sign_request, get_age, get_msmnt_cnt, Submission
import random
...
class Probe:
    ...
    def submit_measurement(self):
        nym_scope = self.nym_scope.replace("{probe_cc}", self.get_cc()).replace("{probe_asn}", self.get_asn())
        ...
        new_cred_sign_request = compute_cred_sign_request(self.age, self.msmnt_count, get_nym_id(self.credential))
