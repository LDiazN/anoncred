...
from .models import create_db_and_tables, init_server_state, ServerState
...
def register(request: RegisterRequest, session: SessionDep, state : ServerStateDep):
    ...
    return RegisterResponse(credential_sign_response=to_str(result), emission_date=state.today())
